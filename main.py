...
#!!!print(f"{show_head_tsl(5)}\n{check_missing_values()}\n{show_basic_stats_tsl()}") /n#2 This was made into a comment since this is not needed at the moment
...
#!!!access_closing_price_with_date(tsla_history) /n#2 This was made into a comment since this was already plotted, no need to do it every runtime
...
def calculation_moving_averages_SMA_50(Ticker):
    ...
    plt.title(f'{Ticker} Stock SMA_50 Prices')
    plt.xlabel('Date')
    plt.ylabel('Price (USD)')
    plt.grid(True)
...
calculation_moving_averages_SMA_200(tsla_history_1y)
